...
resourcename="bob"  # lower case, only
resourcegroup="bob_rg"
...
# git clone https://github.com/apilogicserver/bob.git
# cd bob
...
az group create --name bob_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group bob_rg --sku S1 --is-linux
az container create --resource-group bob_rg --name bob --image apilogicserver/bob:latest --dns-name-label bob --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//bob.westus.azurecontainer.io:5656'
